...
def init_db() -> None:
    ...
    conn.executemany(
        ...
        [
            ...
            ("tutor",    "Tutor",      6980,   80,
             '{"research":true,"compare":true,"save":true,"analysis":true,"students":true}'),
            ("school",   "School",    19800,  300,
             '{"research":true,"compare":true,"save":true,"analysis":true,"students":true,"team":true}'),
        ],
    )
    conn.commit()

    # ── BtoC プラン追加マイグレーション（既存DBに安全に追加） ────────────────
    for code, name, price, quota, features in [
        ("standard", "スタンダード", 1980,  8,
         '{"research":true,"compare":"light","save":true,"diagnosis":false,"analysis":false}'),
        ("premium",  "プレミアム",   3980, 20,
         '{"research":true,"compare":true,"save":true,"diagnosis":true,"analysis":true}'),
        ...
        try:
            conn.execute(
                "INSERT OR IGNORE INTO plans (code, name, price_monthly, monthly_quota, features_json) VALUES (?,?,?,?,?)",
                (code, name, price, quota, features),
            )
        except Exception:
            pass
    conn.commit()
