...
async def startup():
    import logging
    _log = logging.getLogger("ao.startup")

    # 本番環境での必須環境変数チェック
    if ENVIRONMENT == "production":
        missing = []
        if not os.environ.get("DATA_DIR"):
            missing.append("DATA_DIR (例: /data) — 未設定だとデプロイのたびにDBが消えます")
        if not os.environ.get("RESEND_API_KEY"):
            missing.append("RESEND_API_KEY — 未設定だとメール認証が機能しません")
        if not os.environ.get("STRIPE_SECRET_KEY"):
            missing.append("STRIPE_SECRET_KEY — 未設定だと決済が機能しません")
        if not os.environ.get("STRIPE_WEBHOOK_SECRET"):
            missing.append("STRIPE_WEBHOOK_SECRET — 未設定だとWebhook署名検証が失敗します")
        if not os.environ.get("JWT_SECRET_KEY"):
            missing.append("JWT_SECRET_KEY — 未設定だと再起動のたびに全セッションが無効化されます")
        for item in missing:
            _log.critical("【起動警告】環境変数が未設定: %s", item)
...
async def robots_txt():
    return PlainTextResponse(
        ...
        "Sitemap: https://ao.helphero.jp/sitemap-research.xml\n"
    )


@app.get("/sitemap-research.xml", include_in_schema=False)
async def sitemap_research_xml():
    """完了済みリサーチの公開シェアページをGoogleに通知する動的サイトマップ"""
    from database import get_db
    db = get_db()
    try:
        rows = db.execute(
            """SELECT id, updated_at FROM research_requests
               WHERE status='done' ORDER BY updated_at DESC LIMIT 1000"""
        ).fetchall()
    finally:
        db.close()

    urls = "\n".join(
        f'  <url><loc>https://ao.helphero.jp/share/{r["id"]}</loc>'
        f'<lastmod>{(r["updated_at"] or "")[:10]}</lastmod>'
