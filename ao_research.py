            ...
            if not _found_year_url:
                ...
                if not _admission_pages:
                    ...
                    _retry_raw = _call_with_timeout(
                        collect_search_data,
                        [f"{_uni_name_only} 入試要項 募集要項 {_year} 入学センター"],
...
- quota: 本文中に「募集人員 N名」「定員 N名」「N名程度」「約N名」「若干名」等があれば**必ず `quota` フィールドに原文表記のまま抽出**（difficulty_facts の本文だけで触れて `quota` が「不明」のままは禁止）。2027未公表で過年度のみ判明なら「2027年度未公表、2026年度はN名」と明記
