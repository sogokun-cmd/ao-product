...
def run_university_analysis(client: anthropic.Anthropic, keyword: str, pdf_url: str = "", pdf_text: str = "", progress_cb=None, enable_deep_research: bool = False) -> dict:
    ...
    if pdf_url:
        ...
        def _is_university_pdf(url: str) -> bool:
            ...
            if _preferred_domain and _preferred_domain not in url:
                return False  # preferred_domain確定後は他ドメインを拒否
            return True
    ...
    _parallel_workers = int(os.environ.get("RESEARCH_PARALLEL_SITE_WORKERS", "6"))
...
- quota: 本文中に「募集人員 N名」「定員 N名」「N名程度」「約N名」「若干名」「各ブロックN名」「合計N名」「計N名」等があれば**必ず `quota` フィールドに原文表記のまま抽出**。features / difficulty_facts / selection_phase_1 / selection_phase_2 / eligibility を全て走査してから判断すること（これらの本文に書いてあるのに `quota` が「不明」は禁止）。2027未公表で過年度のみ判明なら「2027年度未公表、2026年度はN名」と明記
...
{{
  "universities": [
    {{
      ...
      "admission_policy": {{
        "summary": "学科別AP要約（500〜800字。公式記述を具体的に引用。不明は「不明」）",
